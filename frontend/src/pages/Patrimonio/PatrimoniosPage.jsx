...
export function PatrimoniosPage() {
    ...
    return (
        <div className='max-w-4xl mx-auto my-5'>
                <h1 className="text-2xl font-bold mb-5">Listado de Patrimonio</h1>
            ...
            <table {...getTableProps()} className="min-w-full bg-burdeos rounded-lg outline outline-black ">
                ...
                <tbody {...getTableBodyProps()}>
                    {rows.map((row, index) => {
                        ...
                        return (
                            <tr key={row.original.id} {...rowProps} onClick={() => navigate(`/patrimonios/${row.original.id}`)} className={`cursor-pointer ${rowClassName}`}>
                                {row.cells.map(cell => (
