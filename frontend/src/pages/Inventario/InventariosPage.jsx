...
export function InventariosPage() {
    ...
    return (
        <div className='max-w-4xl mx-auto my-5'>
            ...
            <table {...getTableProps()} className="min-w-full bg-burdeos rounded-lg outline outline-black">
                ...
                <tbody {...getTableBodyProps()}>
                    {rows.map((row, index) => {
                        ...
                        return (
                            <tr key={key} {...rowProps} onClick={() => navigate(`/inventarios/${row.original.id}`)} className={`cursor-pointer ${rowClassName}`}>
                                {row.cells.map(cell => (
            ...
            {/* <button onClick={() => navigate('/inventario')} className="bg-indigo-500 font-bold p-3 rounded-lg block w-50 mt-3">
                Crear Inventario
            </button> */}
